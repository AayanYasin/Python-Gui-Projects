...
ComputersrpcChoice = ["Rock", "Paper", "Scissor"]
User_Name = "Player"
RPS = 0
RCS = 0
score = None
...
def PCP():
    global RPS
    global RCS
    root_playerChoiceAnswer.config(text="Paper")
...
def disabled():
    ...
    root_paper.config(state=DISABLED)
...
root_rock = Button(root, text="Rock", bg="pink", command=PCR)
root_paper = Button(root, text="Paper", bg="yellow", command=PCP)
root_scissor = Button(root, text="Scissor", bg="silver", command=PCS)
...
root_paper.grid(row=6, column=0)
